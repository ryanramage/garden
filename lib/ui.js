...
exports.createDashboardInstructions = function (el, req, id) {
    $('.note-inner', el).html(
        templates.render('install_popup_dashboard.html', req, {
            dashboard_urls: _.map(window._dashboard_urls, function (url, i) {
                return {id: i, url: url};
            })
        })
    );
    ...
    $('.note-actions .primary', el).click(function (ev) {
        ...
        //parts.query.app_url = window.location.host + baseURL + '/details/' + id;
        parts.query.app_url = window.location.toString();
